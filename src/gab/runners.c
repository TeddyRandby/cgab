...
void repl(const char *module, int flags) {
  ...
  gab_value map = gab_map(gab, 0, 0, nullptr, nullptr);

  gab_value with_hello =
      gab_mapput(gab, map, gab_string(gab, "hello"), gab_string(gab, "world"));

  printf("with hello: %V (at: %V)\n", with_hello, gab_mapat(with_hello, gab_string(gab, "hello")));

  gab_value with_hi = gab_mapput(gab, with_hello, gab_string(gab, "hi"),
                                 gab_string(gab, "joe"));

  printf("with hi: %V\n", with_hi);

  gab_value with_hell = gab_mapput(gab, with_hello, gab_string(gab, "hell"), gab_number(69));

  printf("with hell: %V (at: %V)\n", with_hell, gab_mapat(with_hell, gab_string(gab, "hell")));

  gab_value with_help = gab_mapput(gab, with_hell, gab_string(gab, "help"), gab_number(420));

  printf("with help: %V (at: %V)\n", with_help, gab_mapat(with_help, gab_string(gab, "help")));

  gab_value with_h= gab_mapput(gab, with_help, gab_string(gab, "h"), gab_number(42));

  printf("with h: %V (at: %V)\n", with_h, gab_mapat(with_h, gab_string(gab, "h")));

  gab_value with_he = gab_mapput(gab, with_h, gab_string(gab, "he"), gab_string(gab, "do"));

  printf("with he: %V (at: %V)\n", with_he, gab_mapat(with_he, gab_string(gab, "he")));

  gab_value with_heagain = gab_mapput(gab, with_he, gab_string(gab, "he"), gab_string(gab, "dont"));
