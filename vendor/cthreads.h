...
enum {
  ...
  thrd_busy     = 1, // The requested operation failed because a tesource requested by a test and return function is already in use
...
/* Win32 thread-specific storage specifications. Used internally only in cthreads.h. */
#if defined(_CTHREADS_WIN32_)
    ...
    static tss_dtor_t cthreads_tss_destructors[1088] = { NULL, };
...
static int mtx_timedlock(mtx_t *__restrict mtx, const struct timespec *__restrict ts);
...
static int tss_set(tss_t key, void *val);
    ...
    static void NTAPI cthreads_tss_callback_win32(PVOID h, DWORD dwReason, PVOID pv);
#endif
...
//=============================================================================
    ...
    void cthreads_tss_cleanup_win32(void) {
        ...
        void* value;
        for (iteration = 0 ; iteration < TSS_DESTRUCTOR_ITERATIONS && again > 0 ; iteration++) {
            again = 0;
    ...
    void NTAPI cthreads_tss_callback_win32(PVOID h, DWORD dwReason, PVOID pv) {
        (void)h;
        (void)pv;
        if (cthreads_tss_head != NULL && (dwReason == DLL_THREAD_DETACH || dwReason == DLL_PROCESS_DETACH)) {
        ...
        #endif
    #else
        PIMAGE_TLS_CALLBACK static p_thread_callback __attribute__((section(".CRT$XLB"))) = cthreads_tss_callback_win32;
    #endif
...
int thrd_create(thrd_t *thr, thrd_start_t func, void *arg) {
    ...
    thread_start_info* ti = (thread_start_info*)malloc(sizeof(thread_start_info));
    if (ti == NULL) {
      return thrd_nomem;
    }
    ti->mFunction = func;
...
void tss_delete(tss_t key) {
    #if defined(_CTHREADS_WIN32_)
        struct cthreads_tss_data_win32_t* data = (struct cthreads_tss_data_win32_t*) TlsGetValue (key);
...
int tss_set(tss_t key, void *val) {
        ...
        struct cthreads_tss_data_win32_t* data = (struct cthreads_tss_data_win32_t*)TlsGetValue(key);
        if (data == NULL) {
            data = (struct cthreads_tss_data_win32_t*)malloc(sizeof(struct cthreads_tss_data_win32_t));
            if (data == NULL) {
                return thrd_error;
            }
            data->value = NULL;
